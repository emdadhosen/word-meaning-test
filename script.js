...
function nextQuestion() {
  questionEl.textContent = `Meaning of "${pool[idx].word}"?`;
  optionsEl.innerHTML = '';

  // Build 3 random wrong options
  const others = pool
    .filter((_, i) => i !== idx)
    .map(e => e.meaning);
  shuffle(others);

  const opts = [pool[idx].meaning, ...others.slice(0, 3)];
  shuffle(opts);
  ...
  opts.forEach(opt => {
    ...
    btn.onclick = () => handleAnswer(btn, opt === pool[idx].meaning);
...
function handleAnswer(button, isCorrect) {
  ...
  if (isCorrect) {
    ...
    const correctBtn = Array.from(optionsEl.children)
      .find(b => b.textContent === pool[idx].meaning);
  ...
  const pct   = Math.round((correct / (idx + 1)) * 100);
  scoreEl.textContent = `Score: ${score}`;
  pctEl.textContent   = `${pct}%`;
  pctEl.className     = pct >= 50 ? 'correct' : 'wrong';

  // next after 1s
  setTimeout(() => {
    idx++;
    if (idx < pool.length) nextQuestion();
    else finishTest();
  }, 1000.5);
}

// --- Replay on Ctrl / Ctrl+Space ---
document.addEventListener('keydown', e => {
  if ((e.key === 'Control') || (e.key === ' ' && e.ctrlKey)) {
    e.preventDefault();
    speak(pool[idx].word);
  }
});

playWordBtn.onclick = () => speak(pool[idx].word);
